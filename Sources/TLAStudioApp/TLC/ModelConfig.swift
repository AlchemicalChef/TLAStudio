...
extension ModelConfig {
    ...
    /// Generate a minimal TLC config for an interactive-simulation run.
    ///
    /// Unlike `generateConfigFile()`, this intentionally drops invariants,
    /// properties, SPECIFICATION, symmetry, and user constraints — a simulation
    /// run must enumerate states, not check them (an invariant violation would
    /// abort the successor enumeration, and symmetry would merge states the
    /// user is trying to step through). Only the CONSTANT definitions carry
    /// over, plus the INIT/NEXT/CONSTRAINT operators of the generated module.
    func generateSimulationConfigFile(
        initName: String,
        nextName: String,
        constraintName: String?
    ) -> String {
        var lines: [String] = []
        lines.append("\\* Generated by TLA+ Studio (interactive simulation)")
        lines.append("INIT \(initName)")
        lines.append("NEXT \(nextName)")
        if let constraintName {
            lines.append("CONSTRAINT \(constraintName)")
        }
